...
//       let documentUrls: string[] = [];
//       let type: AddContentType = addContentType;

//       // 1) FILES → upload to Firebase Storage, send URLs to backend (type = "files")
//       if (addContentType === "files" && selectedFiles.length > 0) {
//         const storage = getStorage();
//         for (const file of selectedFiles) {
//           const storageRef = ref(
...
//       // 2) WEBPAGES → just send URLs directly (type = "webpages")
//       else if (addContentType === "webpages" && webpageUrl) {
//         documentUrls = [webpageUrl];
//       }

//       // 3) TEXT → DO NOT upload to Firebase.
//       // We only send text_content to backend (type = "text").
//       else if (addContentType === "text" && manualText) {
//         type = "text";
//         // documentUrls stays empty; backend uses text_content for this type.
//       }
...
//             document_urls: documentUrls,
//             type,
//             text_content: manualText, // used when type = "text"
//           }),
//         }
//       );
...
//       <div className="flex flex-col lg:flex-row justify-between items-start  mb-10 gap-4 bg-gray-100 dark:bg-[#141414] dark:text-white">
//         <div>
//           <h1 className="text-2xl sm:text-4xl font-medium mb-2 sm:mb-4">
//             Knowledge base
//           </h1>
//           <p className=" sm:text-xl font-medium leading-relaxed">
//             Centralize all your company's knowledge in{" "}
...
//               className="bg-white dark:bg-[#1a1a1a] dark:border-gray-700  dark:text-white    rounded-[20px] p-6 pt-9 shadow-sm border border-[#1012141A] hover:shadow-md transition-shadow flex flex-col gap-y-6 relative overflow-hidden"
//               style={{
//                 pointerEvents: actionLoading[`delete-${kb.knowledge_base_id}`]
//                   ? "none"
//                   : "auto",
//                 opacity: actionLoading[`delete-${kb.knowledge_base_id}`]
//                   ? 0.5
//                   : 1,
//               }}
//             >
//               <div className="absolute top-0 right-0">
//                 <div
//                   className={`pl-2 pr-4 py-0.5 rounded-sm text-[10px] font-medium ${
//                     kb.status === "complete"
//                       ? "bg-green-100 text-green-700"
//                       : "bg-yellow-100 text-yellow-700"
//                   }`}
...
//                     {kb.knowledge_base_name}
//                   </h3>
//                   <p className="text-[10px] text-[#646465] dark:text-white truncate">
//                     {kb.knowledge_base_sources?.[0]?.filename || "No files"}
//                   </p>
//                 </div>
//               </div>
...
//                         target="_blank"
//                         rel="noopener noreferrer"
//                         className="p-1.5 text-[#646465] hover:text-gray-600"
//                       >
//                         <Link size={14} />
//                       </a>
...
//                         target="_blank"
//                         rel="noopener noreferrer"
//                         className="p-1.5 text-[#646465] dark:text-white hover:text-gray-600"
//                       >
//                         <Download size={14} />
//                       </a>
//                     ) : null)}
...
//                       size={14}
//                       className={
//                         actionLoading[`delete-${kb.knowledge_base_id}`]
//                           ? "animate-pulse"
//                           : ""
//                       }
//                     />
...
//   /**
//    * NEW IMPLEMENTATION – no Firebase Storage
//    * Sends file / url / text directly to backend as multipart/form-data
//    */
//   const handleCreateKnowledgeBase = async () => {
//     if (!user || !newKbName.trim() || addContentType === "none") return;
...
//       formData.append("knowledge_base_name", newKbName);
//       formData.append("type", addContentType);

//       let hasPayload = false;

//       if (addContentType === "files" && selectedFiles.length > 0) {
//         // send all selected files as "files"[]
//         selectedFiles.forEach((file) => {
//           formData.append("files", file);
//         });
//         hasPayload = true;
//       } else if (addContentType === "webpages" && webpageUrl.trim()) {
//         // backend will parse document_urls (string or array)
//         formData.append("document_urls", webpageUrl.trim());
//         hasPayload = true;
//       } else if (addContentType === "text" && manualText.trim()) {
//         // text goes as text_content; backend will use knowledge_base_texts
//         formData.append("text_content", manualText.trim());
//         hasPayload = true;
//       }

//       if (!hasPayload) {
//         setCreateLoading(false);
//         setError("Please provide a file, URL, or text content.");
//         return;
//       }
...
//       console.error("Error creating knowledge base:", error);
//       setError(
//         error?.message || "Failed to create knowledge base. Please try again.",
//       );
//     } finally {
//       setCreateLoading(false);
//     }
...
export function KnowledgeBase() {
  ...
  const fetchKnowledgeBases = async () => {
    ...
    try {
      ...
      setError(null);
      const response = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/api/knowledge-bases?user_id=${user.uid}&workspace_id=1`
      );
      if (!response.ok) {
        throw new Error("Failed to fetch knowledge bases");
      }
  ...
  /**
   * Create knowledge base WITHOUT Google/Firebase Storage.
   * We send files / URLs / text directly to the backend as FormData.
   */
  const handleCreateKnowledgeBase = async () => {
    if (!user || !newKbName.trim() || addContentType === "none") return;
    ...
    try {
      ...
      formData.append("user_id", user.uid);
      formData.append("workspace_id", "1");
      formData.append("knowledge_base_name", newKbName.trim());
      formData.append("type", addContentType);

      if (addContentType === "files") {
        ...
        selectedFiles.forEach((file) => {
          formData.append("files", file); // matches upload.array("files")
        });
      } else if (addContentType === "webpages") {
        if (!webpageUrl.trim()) {
          setError("Please enter a website URL");
          setCreateLoading(false);
          return;
        }
        // Backend supports either single string or comma-separated URLs
        formData.append("document_urls", webpageUrl.trim());
      ...
      if (!response.ok) {
        ...
        const msg =
          errJson?.error || errJson?.message || "Failed to create knowledge base";
        throw new Error(msg);
      }
    ...
    if (addContentType === "none") {
      return (
        <div className="space-y-2">
          <button
            onClick={() => setAddContentType("webpages")}
            className="w-full flex items-center space-x-2 p-3 text-left hover:bg-gray-50 dark:hover:bg-[#1f2933] rounded-lg transition-colors"
          >
        ...
        return (
          <div>
            ...
            <input
              ...
              onChange={(e) => setWebpageUrl(e.target.value)}
              className="w-full px-2.5 py-1.5 border border-[#1012141A] dark:text-white dark:bg-[#141414] dark:border-gray-700 rounded-[6px] appearance-none outline-none text-xs font-medium text-[#646465]"
              placeholder="https://example.com/your-doc-or-page"
            />
            <p className="mt-1 text-[10px] text-[#646465]">
              You can also paste a direct PDF link here – the backend will fetch it.
            </p>
          </div>
        );
          ...
          <div>
            ...
            <div className="space-y-2">
              {selectedFiles.map((file, index) => (
                <div
                  key={index}
                  className="flex items-center justify-between p-2 bg-gray-50 dark:bg-[#1f2933] rounded"
                >
                  <span className="text-xs truncate">{file.name}</span>
                  <button
                    onClick={() =>
              ...
              <button
                type="button"
                onClick={() => fileInputRef.current?.click()}
                className="w-full h-32 border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-lg flex items-center justify-center hover:border-gray-400 dark:hover:border-gray-500 transition-colors"
              >
                <div className="text-center">
                  <Upload size={24} className="mx-auto mb-2 text-gray-400" />
  ...
  return (
    <div className="px-2 sm:px-4 py-4 max-w-screen-xl mx-auto">
      <div className="flex flex-col lg:flex-row justify-between items-start mb-10 gap-4 bg-gray-100 dark:bg-[#141414] dark:text-white rounded-xl p-4 sm:p-6">
        <div>
          <h1 className="text-2xl sm:text-4xl font-medium mb-2 sm:mb-4">
            Knowledge base
          </h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {knowledgeBases.map((kb) => (
            ...
            >
              ...
              <div className="flex items-start space-x-3">
                <div className="w-10 h-10 bg-gray-100 dark:bg-[#111827] rounded-lg flex items-center justify-center flex-shrink-0">
                  📄
                </div>
                <div className="flex-1 min-w-0">
                  <h3 className="text-xs font-medium truncate mb-1">
                  ...
                  <p className="text-[10px] text-[#646465] dark:text-gray-300 truncate">
                    {kb.knowledge_base_sources?.[0]?.filename || "No files"}
                  </p>
                </div>
              </div>

              <div className="flex items-center justify-between text-[10px] text-[#646465] dark:text-gray-300">
                <div>Pages: {kb.knowledge_base_sources?.length || 0}</div>
                <div className="flex gap-2">
                  {kb.knowledge_base_sources?.[0] && (
                    kb.knowledge_base_sources[0].type === "url" ? (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="p-1.5 text-[#646465] dark:text-gray-300 hover:text-gray-600"
                      >
                        <LinkIcon size={14} />
                      </a>
                    ) : (kb.knowledge_base_sources[0].type === "text" ||
