...
export function Dashboard() {
  ...
  return (
    <div className="dark:bg-[#141414] dark:text-white min-h-screen px-2 sm:px-4 py-4 bg-white text-black max-w-screen-xl mx-auto">
      <h1 className="text-2xl sm:text-3xl font-semibold mb-1">Welcome {capFirst(firstName)}!</h1>
      <p className="page-subtitle mb-10 text-sm sm:text-base">
        Let’s save you <span className="bg-[#155EEF] text-white px-2 py-0.5 rounded">68%</span> on payroll and schedule more appointments.
      </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        ...
        <div className="bg-white dark:bg-[#1a1a1a] rounded-[20px] p-6 shadow-sm border border-[#e5e7eb] dark:border-gray-700">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-md font-medium">Agents</h2>
            <button onClick={() => navigate('/agents')} className="w-6 h-6 bg-[#155EEF] rounded-full flex items-center justify-center hover:opacity-90">
              <Plus size={14} className="text-white" />
            </button>
          </div>
          {loading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 mb-6">
              {agents.slice(0, 6).map((agent, index) => (
                <motion.div custom={index} variants={fadeInUp} initial="hidden" animate="visible" whileHover={{ scale: 1.09 }} key={agent.id} className="flex flex-col hover:shadow-sm transition-all duration-200 rounded-lg p-2">
                  <div className="flex items-center">
                    <img src={agent.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(agent.agent_name || 'Agent')}`} alt={agent.agent_name} className="rounded-full mr-3 w-10 h-10" />
                    <div>
                      <h3 className="text-xs font-medium">{agent.agent_name || 'Agent'}</h3>
                      <div className="text-xs text-gray-500">Phone: {agent.phone || 'N/A'}</div>
                      <div className="text-xs text-gray-500">Voice: {agent.voice || 'N/A'}</div>
                    </div>
                  </div>
                </motion.div>
              ))}
          ...
          <motion.button whileTap={{ scale: 0.95 }} whileHover={{ scale: 1.05 }} onClick={() => navigate('/agents')} className="bg-blue-600 hover:bg-blue-700 text-white py-1.5 px-4 rounded-lg text-xs">
            Go to Agents
          </motion.button>
        </div>
        ...
        <div className="bg-white dark:bg-[#1a1a1a] rounded-[20px] p-6 shadow-sm border border-[#e5e7eb] dark:border-gray-700 flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-md font-medium">Knowledge base</h2>
              <button onClick={() => navigate('/knowledge-base')} className="w-6 h-6 bg-[#155EEF] rounded-full flex items-center justify-center hover:opacity-90">
                <Plus size={14} className="text-white" />
              </button>
            </div>
            {loading ? (
                ...
                {knowledgeBases.slice(0, 3).map((kb) => (
                  <div key={kb.knowledge_base_id} className="border-b border-b-[#155EEF1A] pb-4 last:border-b-0">
                    <div className="flex items-center justify-between">
                      <div className="flex items-start flex-col">
                        <div className="flex items-center mb-1">
                          <div className="mr-3 bg-[#F6F6F6] p-1.5 rounded-full">
                            <img src={Details} alt="details-icon" />
                          </div>
                          <div className="font-medium text-sm">{kb.knowledge_base_name}</div>
                        </div>
                      ...
                      {kb.knowledge_base_sources?.[0] && (
                        <div className="flex gap-2 self-center">
                          {kb.knowledge_base_sources[0].type === "url" ? (
                            <a href={kb.knowledge_base_sources[0].url} target="_blank" rel="noopener noreferrer" className="p-1 text-[#646465] hover:text-gray-600 transition-colors">
                              <Link size={14} />
                            </a>
                            ...
                            <a href={kb.knowledge_base_sources[0].file_url || kb.knowledge_base_sources[0].content_url} target="_blank" rel="noopener noreferrer" className="p-1 text-[#646465] hover:text-gray-600 transition-colors">
                              <Download size={14} />
                            </a>
                          ) : null}
                        </div>
          ...
          <motion.button whileTap={{ scale: 0.95 }} whileHover={{ scale: 1.05 }} onClick={() => navigate('/knowledge-base')} className="bg-blue-600 hover:bg-blue-700 text-white py-1.5 px-4 rounded-lg text-xs">
            Go to Knowledge base
          </motion.button>
        </div>
      </div>
